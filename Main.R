...
filtered_guarantor <- guarantors_ent %>% filter(!is.na(guarantorsname))
num_rows_to_add <- nrow(filtered_guarantor)
...
NDG_ent <- NDG_ent %>%
  mutate(name = ifelse(row_number() > (nrow(NDG_ent) - num_rows_to_add), filtered_guarantor$guarantorsname, name),
         cf.piva = ifelse(row_number() > (nrow(NDG_ent) - num_rows_to_add), filtered_guarantor$taxcodeforguarantors, cf.piva),
         id.bor = ifelse(row_number() > (nrow(NDG_ent) - num_rows_to_add), filtered_guarantor$id.bor, id.bor)) %>%
  distinct()
...
filtered_guarantor_count <- guarantors_count %>% filter(!is.na(guarantorsname))
num_rows_to_add_c <- nrow(filtered_guarantor_count)
nc_data <- data.frame(name = rep(NA, num_rows_to_add_c))
NDG_ent <- bind_rows(NDG_ent, nc_data)
NDG_count <- bind_rows(NDG_count, nc_data)
NDG_count$name[(nrow(NDG_count) - num_rows_to_add_c + 1):nrow(NDG_count)] <- filtered_guarantor_count$guarantorsname
NDG_count$cf.piva[(nrow(NDG_count) - num_rows_to_add_c + 1):nrow(NDG_count)] <- filtered_guarantor_count$taxcodeforguarantors
NDG_count$id.bor[(nrow(NDG_count) - num_rows_to_add_c + 1):nrow(NDG_count)] <- filtered_guarantor_count$id.bor
NDG_count<- NDG_count
NDG_count$role <- ifelse(is.na(NDG_count$role), "Guarantor", NDG_count$role)
